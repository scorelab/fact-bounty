...
class NewsFirstSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.newsfirst.lk/category/life/",
    ...
    def __init__(self, date=None, *args, **kwargs):
        super(NewsFirstSpider, self).__init__(*args, **kwargs)
    ...
    def parse_article(self, response):
        ...
        if (
            response.css(".artical-new-byline::text").extract()
            and response.css(".artical-new-byline::text").extract()[1]
        ):
        ...
        item["imageLink"] = response.css(
            ".main-news-block-artical .img-responsive::attr(src)"
        ).extract_first()
        item["source"] = "https://www.newsfirst.lk"
        item["content"] = "\n".join(
            response.css(".editor-styles p::text").extract()
        )
