...
function HoaxyGraph(options) {
  ...
  var spinStart =
    options.spinStart ||
    function() {
      console.log('HoaxyGraph.spinStart is undefined.')
    }
  var spinStop =
    options.spinStop ||
    function() {
      console.log('HoaxyGraph.spinStop is undefined.')
    }
  var toggle_edge_modal =
    options.toggle_edge_modal ||
    function() {
      console.log('HoaxyGraph.toggle_edge_modal is undefined.')
    }
  var toggle_node_modal =
    options.toggle_node_modal ||
    function() {
      console.log('HoaxyGraph.toggle_node_modal is undefined.')
    }
  ...
  var graphAnimation = options.graphAnimation || {
    playing: false,
    increment: 0,
    total_increments: 40
  }
  ...
  function getBotCacheScores() {
    ...
    for (var i in edges) {
      ...
      var from_user_id = edge.from_user_id, // eslint-disable-line no-unused-expressions
  ...
  function getNewBotometerScore(user_id) {
    ...
    botScoreA = new Promise(function(resolve, reject) {
        ...
        function(resolve, reject) {
          ...
          user_data
            .then(
              function(response) {
                user.user = response
                var user_mentions = twitter.getUserMentions(
      ...
      var got_from_twitter = Promise.all([
        user_data_and_mentions,
        user_timeline
      ])
      got_from_twitter.then(
        function(values) {
        ...
        function(error) {
          console.warn(
            'Could not get bot score for ' + screen_name + ': ',
            error
          )
          // score_stats.recompute();
  ...
  function getBotScore(user_object, potentially_old_sn) {
    ...
    botscore.then(
      function(response) {
        ...
        var newScore = 0
        if (
          response.data.user.lang === 'en' ||
          response.data.user.lang === 'en-gb'
        ) {
        ...
        var completeAutomationProbability = Math.round(
          response.data.cap.english * 100
        )
        ...
        botscores[id] = {
          ...
          screen_name: sn,
          completeAutomationProbability: completeAutomationProbability,
          staleAcctInfo: {
            isStale: isStale,
            newId: newId,
            oldSn: oldSn,
  ...
  function UpdateGraph(start_time, end_time) {
    ...
    try {
      ...
      for (var i in edges) {
        ...
        var url_raw = edge.url_raw,
          title = edge.title

        //sanity check
        //if -1, dump it since no  out edge
        if (-1 == to_user_id) continue
        nodes[from_user_id] = nodes[from_user_id] || {
          id: '',
          size: 1,
          color: '',
          incomingCount: 0,
        ...
        nodes[from_user_id].outgoing[to_user_id] = nodes[from_user_id].outgoing[
          to_user_id
        ] || {
          type: '',
          fact_checking: 0,
          ...
          screenName: '',
          count: 0,
          min_tweet_created_at: new Date().getTime(),
          max_tweet_created_at: 0,
          is_mentions: [],
          tweet_types: [],
          ids: [],
          url_raws: [],
        ...
        nodes[from_user_id].outgoing[to_user_id].type =
          nodes[from_user_id].outgoing[to_user_id].fact_checking >
          nodes[from_user_id].outgoing[to_user_id].claim
            ? 'fact_checking'
            : 'claim'

        nodes[from_user_id].outgoing[to_user_id].screenName =
          edge.to_user_screen_name
        nodes[from_user_id].outgoing[to_user_id].count++

        nodes[from_user_id].outgoing[
          to_user_id
        ].min_tweet_created_at = Math.min(
          tweet_created_at,
          nodes[from_user_id].outgoing[to_user_id].min_tweet_created_at
        )
        nodes[from_user_id].outgoing[
          to_user_id
        ].max_tweet_created_at = Math.max(
          tweet_created_at,
          nodes[from_user_id].outgoing[to_user_id].max_tweet_created_at
        )
        ...
        nodes[to_user_id] = nodes[to_user_id] || {
          id: '',
          size: 1,
          color: '',
          incomingCount: 0,
        ...
        nodes[to_user_id].incoming[from_user_id] = nodes[to_user_id].incoming[
          from_user_id
        ] || {
          type: '',
          fact_checking: 0,
        ...
        nodes[to_user_id].incoming[from_user_id].type =
          nodes[to_user_id].incoming[from_user_id].fact_checking >
          nodes[to_user_id].incoming[from_user_id].claim
            ? 'fact_checking'
            : 'claim'

        nodes[to_user_id].incoming[from_user_id].screenName =
          edge.from_user_screen_name
        ...
        edgeCount[from_user_id + ' ' + to_user_id] =
          edgeCount[from_user_id + ' ' + to_user_id] || 0
      ...
      // Then we add the bot scores that are stale or never obtained
      var toComputeLen = account_list_to_compute.length
      for (var i = 0; i < toComputeLen; i++) {
        score_stats.user_list.push(account_list_to_compute[i])
      }
  ...
  function GenerateUserModal(e) {
    ...
    for (var i in node.incoming) {
      ...
      for (var j in node.incoming[i].ids) {
        var tweetURL = TWEET_URL.replace('%0', i).replace(
          '%1',
          node.incoming[i].ids[j]
        )
        if (
          true !== node.incoming[i].is_mentions[j] &&
          false !== node.incoming[i].is_mentions[j]
        )
        ...
        if (
          true === node.incoming[i].is_mentions[j] ||
          'reply' === node.incoming[i].tweet_types[j]
        ) {
        ...
        tweets[tweet_type][i] = tweets[tweet_type][i] || {
          user_url: fromURL,
          screenName: node.incoming[i].screenName,
          article_titles: [],
          tweet_urls: [],
          article_urls: []
        }
    ...
    for (var i in node.outgoing) {
      ...
      for (var j in node.outgoing[i].ids) {
        var tweetURL = TWEET_URL.replace('%0', i).replace(
          '%1',
          node.outgoing[i].ids[j]
        )
        if (
          true !== node.outgoing[i].is_mentions[j] &&
          false !== node.outgoing[i].is_mentions[j]
        )
        ...
        if (
          true === node.outgoing[i].is_mentions[j] ||
          'reply' === node.outgoing[i].tweet_types[j]
        ) {
        ...
        tweets[tweet_type][i] = tweets[tweet_type][i] || {
          user_url: toURL,
          screenName: node.outgoing[i].screenName,
          article_titles: [],
          tweet_urls: [],
          article_urls: []
        }
  ...
  function drawGraph() {
    ...
    s.bind('clickNode', function(e) {
      ...
      node_modal_content.staleAcctInfo.openedModalWhileFetchingScores =
        getting_bot_scores.running
      ...
      if (botscores[node.id]) {
        ...
        node_modal_content.botcolor =
          score !== 0 ? getNodeColor(score / 100) : ''
        ...
        if (
          botscores[node.id].hasOwnProperty('staleAcctInfo') &&
          botscores[node.id].hasOwnProperty('completeAutomationProbability')
        ) {
          node_modal_content.staleAcctInfo.isStale =
            botscores[node.id].staleAcctInfo.isStale
          node_modal_content.staleAcctInfo.newId =
            botscores[node.id].staleAcctInfo.newId
          node_modal_content.staleAcctInfo.oldSn =
            botscores[node.id].staleAcctInfo.oldSn
          node_modal_content.staleAcctInfo.newSn =
            botscores[node.id].staleAcctInfo.newSn

          // updating the CAP score
          node_modal_content.completeAutomationProbability =
            botscores[node.id].completeAutomationProbability
          node_modal_content.showStaleContent = true
        } else {
    ...
    s.bind('clickEdge', function(e) {
      ...
      for (var i = 0; i < edge.outgoing_ids.length; ++i) {
        ...
        type === 'reply' ? (type = 'mention') : type // eslint-disable-line no-unused-expressions
        type === 'origin' ? (type = 'mention') : type // eslint-disable-line no-unused-expressions
        ++tweet_types_hashtable[type]
        tweet_urls[edge.outgoing_ids[i]] = TWEET_URL.replace(
          '%0',
          edge.target
        ).replace('%1', edge.outgoing_ids[i])
      }
      ...
      for (var key in tweet_types_hashtable) {
        if (
          tweet_types_hashtable.hasOwnProperty(key) &&
          tweet_types_hashtable[key] > 0
        ) {
          0 === elemNum++ ? (label_string += ' ') : (label_string += ', ')
          if ('mention' === key) label_string += 'mentioned'
          else if ('retweet' === key) label_string += 'was retweeted by'
          else if ('quote' === key) label_string += 'was quoted by'
        }
      }
  ...
  function FilterEdges(filterTimestamp) {
    ...
    for (var i in edges) {
      ...
      if (edge.min_tweet_created_at >= filterTimestamp) {
        ...
        //not filtered
        unfiltered_node_edge_counts[edge.target] =
          unfiltered_node_edge_counts[edge.target] || 0
        unfiltered_node_edge_counts[edge.source] =
          unfiltered_node_edge_counts[edge.source] || 0
    ...
    for (var i in nodes) {
      ...
      if (unfiltered_nodes.indexOf(node.id) === -1) {
        ...
        var percent =
          Math.sqrt(unfiltered_node_edge_counts[node.id]) / Math.sqrt(max_size)
        var new_size = percent * 1000 + 1
        if (new_size < 300) {
          new_size = 300
        }
  ...
  function AnimateFilter(timestamp) {
    ...
    if (
      timestamp > timespan.end_time ||
      graphAnimation.increment > graphAnimation.total_increments
    ) {
    ...
    var increment =
      (timespan.end_time - timespan.start_time) /
      graphAnimation.total_increments
  ...
  returnObj.getEdges = function() {
    return edges
  }
  returnObj.botscores = function() {
    return botscores
  }
  returnObj.resetBotscores = function() {
    botscores = {}
  }
  returnObj.setLang = function(passedLang) {
    lang = passedLang
  }
