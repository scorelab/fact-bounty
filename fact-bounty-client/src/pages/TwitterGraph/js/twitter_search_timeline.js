...
function TwitterSearchTimeline(settings) {
  ...
  chart.interactiveLayer.tooltip.contentGenerator(function(chartData) {
    ...
    for (dateRateIx in chartDataWithTweetRates[0].values) {
      var dateRateMatch = new Date(
        chartDataWithTweetRates[0].values[dateRateIx].x
      ).getTime()
  ...
  function redraw() {
    if (chart) {
      ...
      d3.selectAll('#chart svg .nv-axisMax-x text').attr(
        'transform',
        'translate(0, 12)'
      )
  ...
  function calculateTweetRates(chartData) {
    ...
    for (var i = 0; i < numTimeIncrements; i++) {
      ...
      chartDataWithTweetRates[0].values[i].y =
        currentTweets - previousTimestepTweets
  ...
  function _updateDateRange(extent) {
    ...
    if (document.getElementById('extent-00'))
      document.getElementById('extent-00').innerHTML = new Date(
        extent.extent[0]
      ).toISOString()
    if (document.getElementById('extent-11'))
      document.getElementById('extent-11').innerHTML = new Date(
        extent.extent[1]
      ).toISOString()
...
export default TwitterSearchTimeline
